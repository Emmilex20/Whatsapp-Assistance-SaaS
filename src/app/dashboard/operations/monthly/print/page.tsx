...
export default async function PrintableMonthlyReportPage() {
  ...
  if (!restaurant || !report) return null;
      ...
      <div className="mx-auto max-w-4xl space-y-6 rounded-3xl bg-white p-8 print:rounded-none print:p-6">
        ...
        <section className="grid grid-cols-2 gap-4 md:grid-cols-4">
          {[
            ...
            { label: "Campaign posts", value: report.campaignPosts.length },
          ].map((item) => (
            <div
              key={item.label}
        ...
        <section>
          <h2 className="text-lg font-bold">Campaign posts this month</h2>
        ...
        <section className="grid gap-4 md:grid-cols-2">
          <div className="rounded-2xl border border-zinc-200 p-5">
            ...
            <p className="mt-2 text-sm text-zinc-600">
              Events: {report.ai.events}
            </p>
            <p className="mt-1 text-sm text-zinc-600">
              Tokens: {report.ai.tokens.toLocaleString()}
            </p>
            <p className="mt-1 text-sm text-zinc-600">
              Estimated cost: ${report.ai.cost.toFixed(4)}
            </p>
          </div>

          <div className="rounded-2xl border border-zinc-200 p-5">
            <h2 className="text-lg font-bold">Media usage</h2>
            <p className="mt-2 text-sm text-zinc-600">
              Generated assets: {report.media.generations}
            </p>
            <p className="mt-1 text-sm text-zinc-600">
              Estimated cost: ${report.media.cost.toFixed(4)}
            </p>
          </div>
        </section>
        ...
        <section className="rounded-2xl bg-emerald-50 p-5">
          <h2 className="text-lg font-bold">Monthly insight</h2>
